    ...
    <div className={`min-h-screen w-full p-3 sm:p-4 md:p-5 lg:p-6 relative flex bg-app ${isTrek ? 'overflow-hidden' : 'flex-col'} short-landscape:overflow-y-auto short-landscape:h-auto`} data-theme={settings.theme || 'dark'}>
      ...
      <div className={`flex-1 flex flex-col ${isTrek ? 'mt-2' : ''}`}>
        ...
        <div className="relative z-10 flex-1 flex flex-col gap-4 min-h-0">
          <div className="flex-shrink-0 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 auto-rows-[200px] sm:auto-rows-[180px] md:[grid-template-rows:clamp(200px,30vh,340px)] ipad-landscape:[grid-template-rows:clamp(180px,26vh,280px)] tablet-landscape:[grid-template-rows:clamp(180px,26vh,280px)]">
