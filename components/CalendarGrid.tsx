  ...
  return (
    <div className="h-full flex flex-col">
        ...
        <div className="flex justify-between items-center mb-4 px-1">
            ...
            {!isGoogleLinked && (
                <button 
                    ...
                    className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 hover:bg-white/20 transition-colors text-xs font-bold text-white"
                >
        ...
        {showInput && !isGoogleLinked && (
            <div className="mb-6 mx-1 p-3 bg-black/40 rounded-2xl border border-white/10 backdrop-blur-md">
                <textarea 
                    value={input}
                    onChange={(e) => setInput(e.target.value)}
                    placeholder="e.g., 'Grandma coming for dinner Sunday at 5pm'"
                    className="w-full bg-transparent text-white text-lg focus:outline-none resize-none placeholder-white/30 mb-2 font-light"
                    rows={1}
                />
                <div className="flex justify-end">
                    <button 
                        ...
                        className="px-4 py-2 rounded-xl bg-blue-600 text-xs font-bold text-white hover:bg-blue-500 transition-colors disabled:opacity-50"
                    >
        ...
        <div className="grid grid-cols-4 gap-4 flex-1 min-h-0">
            {days.map((day, i) => {
                ...
                return (
                    ...
                    >
                        {/* Column Header */}
                        <div className={`p-4 border-b border-white/5 ${isToday ? 'bg-blue-500/10' : ''}`}>
                            <h3 className={`text-lg font-bold ${isToday ? 'text-blue-300' : 'text-white'}`}>
                        ...
                        <div className="flex-1 p-3 overflow-y-auto space-y-2 custom-scrollbar">
                            {dayEvents.length === 0 ? (
                                <div className="h-full flex flex-col items-center justify-center text-white/10">
                                    ...
                                    <span className="text-[10px] font-medium uppercase tracking-widest">No Events</span>
                                </div>
                            ) : (
                                dayEvents.map((event) => {
                                    ...
                                    return (
                                        <div 
                                            key={`${event.id}_${i}`} 
                                            className={`p-3 rounded-xl border ${getColorStyles(event.color)} relative group transition-all hover:scale-[1.02]`}
                                        >
                                            <div className="flex items-start justify-between mb-1">
                                                ...
                                                {multiDayLabel && (
                                                    <span className="text-[9px] opacity-50 bg-black/20 px-1.5 rounded-md">
                                                        {multiDayLabel}
                                                    </span>
                                                )}
                                            </div>
